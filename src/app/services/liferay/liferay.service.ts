...
export class LiferayService {
  ...
  getRawData() {    
	...
	this.http.get(
      ` http://localhost:8380/o/headless-delivery/v1.0/sites/34809/structured-contents?filter=contains(title, 'Screens - Self Service')`, {headers})
      .subscribe((data) => {
  ...
  getLocalisedContent() {    
	...
	const dataSub = new Subject<string>();
	this.http.get(
      `http://localhost:8380/o/headless-delivery/v1.0/sites/34809/structured-contents?filter=contains(title, 'Pantallas - Autoservicio')`, {headers})
      .subscribe((data) => {
		dataSub.next(data['items'][0]['contentFields'][0].value.data);
      }, (err) => {
        console.log(err);
